...
def encode_data(filename):
    ...
    reduced_attacks = {"normal": "normal", "neptune": "dos",
                       "warezclient": "r2l", "ipsweep": "probe",
                       "portsweep": "probe", "teardrop": "dos",
                       "nmap": "probe", "satan": "probe",
                       "smurf": "dos", "pod": "dos",
                       "back": "dos", "guess_passwd": "r2l",
                       "ftp_write": "r2l", "multihop": "r2l",
                       "rootkit": "u2r", "buffer_overflow": "u2r",
                       "imap": "r2l", "warezmaster": "r2l",
                       "phf": "r2l", "land": "dos",
                       "loadmodule": "u2r", "spy": "r2l"}
    train_data[41].replace(reduced_attacks, inplace=True)
    train_data.to_csv(filename + ' reducedAttacks_data.csv', sep=',',
                      encoding='utf-8', index=False, header=False)
    attck_encode = {"normal": 0, "dos": 1, "r2l": 2, "probe": 3, "u2r": 4}
    train_data[41].replace(attck_encode, inplace=True)
    train_data.to_csv(filename + ' encoded_data.csv', sep=',', encoding='utf-8',
                      index=False, header=False)
    two_type_encode = {0: 0, 1: 1, 2: 1, 3: 1, 4: 1}
    train_data[41].replace(two_type_encode, inplace=True)
    train_data.to_csv(filename + ' bolean_attack.csv', sep=',', encoding='utf-8',
                      index=False, header=False)
...
def slected_feature_data_sets_save(ind, filename):
    ...
    train_dat.to_csv(filename + ' feature selected.csv', sep=',', encoding='utf-8',
                     index=False, header=False)
...
def find_features():
    ...
    target_df = train_dat[41]
    print(ind[ind.__len__() - 8:ind.__len__()])
    ind = ind[0:ind.__len__() - 8]
    train_dat = train_dat.drop(ind, axis=1)
    train_dat = train_dat.drop(42, axis=1)
    train_dat = pd.concat([train_dat, target_df], axis=1)
    train_dat.to_csv('./data/20 percent train set boolean feature selected.csv', sep=',', encoding='utf-8',
                     index=False, header=False)
...
def normalized_data(filename):
    ...
    data.columns = ["service", "flag", "src bytes", "dst bytes", "same srv rate",
                    "diff srv rate", "dst host srv count", "dst host same srv rate", "target"]
    data.to_csv('./data/final/' + filename + ' with normalized data' + '.csv', sep=',', encoding='utf-8',
...
def main():
    ...
    find_features()
    normalized_data('20 Percent Training Set reducedAttacks_data feature selected')
    normalized_data('20 Percent Training Set encoded_data feature selected')
